# 🛤️ Trilha: Carreira Back-End Node.js — Alura

> Início: **29/07/2026**

Formato: cada nível/curso/aula tem a data em que foi iniciado e concluído.
Uso `[x]` pra concluído, `[ ]` pra pendente, e `🟡` pra "em andamento".

---

## 📦 Base - Lógica de programação e JavaScript
`Início: 29/07/2026` · 2 cursos · 28h

### Curso 1 - JavaScript: aprendendo a programar 🟡
`Início: 29/07/2026`

- [x] Aula 01 - Primeiros passos `29/07/2026 -> 30/07/2026`
- [x] Aula 02 - Algoritmos e pensamento computacional `30/07/2026 -> 31/07/2026` 
- [x] Aula 03 - Configurando ambiente de desenvolvimento `31/07/2026 -> 31/07/2026`
- [x] Aula 04 - Variáveis e tipos de dados `31/07/2026 -> 31/07/2026`
- [x] Aula 05 - Operadores`31/07/2026 -> 02/08/2026`
- [x] Aula 06 - Condições e fluxos `03/08/2026 -> 03/08/2026`
- [x] Aula 07 - Laços, operadores e decisão `03/08/2026 -> 05/08/2026`
- [x] Aula 08 - Funções `05/08/2026 -> 13/08/2026`
- [x] Aula 09 - Arrays `14/08/2026 -> 16/08/2026`
- [x] Aula 10 - Objetos `16/08/2026 -> 24/08/2026`
- [🟡] Aula 11 - Fundamentos modernos do JavaScript `24/08/2026 ->`
 
### Curso 2 - Git e GitHub: compartilhando e colaborando em projetos
- [ ] (aulas ainda não iniciadas)

---

## 📦 Nível 1 - Construindo APIs completas
- [ ] (curso(s) ainda não iniciado(s))

---

## 📦 Nível 2 - Banco de dados, performance, segurança e novos padrões de API
- [ ] (curso(s) ainda não iniciado(s))

---

## 📦 Nível 3 - Arquitetura e sistemas distribuídos
- [ ] (curso(s) ainda não iniciado(s))

---

### 📊 Progresso geral
| Nível | Status | Progresso |
|---|---|---|
| Base | 🟡 Em andamento | 1/2 cursos |
| Nível 1 | ⬜ Não iniciado | 0% |
| Nível 2 | ⬜ Não iniciado | 0% |
| Nível 3 | ⬜ Não iniciado | 0% |
